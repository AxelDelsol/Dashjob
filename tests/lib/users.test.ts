import { INVALID_EMAIL, PASSWORD_INVALID_MATCH, PASSWORD_NO_DIGIT, PASSWORD_NO_LOWER, PASSWORD_NO_SPECIAL, PASSWORD_NO_UPPER, PASSWORD_TOO_SHORT, UserCreate } from '@/lib/users';
import { z } from "zod";
...
describe('UserCreate schema', () => {
  ...
  it('rejects when password and confirmedPassword are differents', () => {
    const data = { email: 'test@example.com', password: 'lower', confirmedPassword: 'bar' }
...
function expect_invalid_field(schema: z.ZodTypeAny, data: object, error_field: string, error_message: string) {
